...
function companiesDataset(companies: Company[]): ReportDataset<Company> {
  ...
  const columns: ReportColumn<Company>[] = [
    ...
    { key: 'iec_code', label: 'IE Code', group: 'Export', type: 'text', get: (c) => c.iec_code },
    { key: 'exporter_type', label: 'Exporter Type', group: 'Export', type: 'label', get: (c) => c.exporter_type },
    { key: 'aepc_code', label: 'AEPC Code', group: 'Export', type: 'text', get: (c) => c.aepc_code },
    { key: 'textile_committee_code', label: 'Textile Committee', group: 'Export', type: 'text', get: (c) => c.textile_committee_code },
    { key: 'msme_number', label: 'MSME / Udyam', group: 'Statutory', type: 'text', get: (c) => c.msme_number },
    { key: 'epf_number', label: 'EPF No.', group: 'Statutory', type: 'text', get: (c) => c.epf_number },
    { key: 'esi_number', label: 'ESI No.', group: 'Statutory', type: 'text', get: (c) => c.esi_number },
    { key: 'professional_tax_number', label: 'Professional Tax No.', group: 'Statutory', type: 'text', get: (c) => c.professional_tax_number },
    { key: 'directors_list', label: 'Directors', group: 'Directors', type: 'text', defaultHidden: true, get: (c) => (c.directors ?? []).filter((d) => d.is_active).map((d) => d.name).join(', ') },
    { key: 'directors_dsc', label: 'Director DSCs', group: 'Directors', type: 'text', defaultHidden: true, get: (c) => (c.directors ?? []).filter((d) => d.dsc_number).map((d) => `${d.name}: ${d.dsc_number}`).join('; ') },
    { key: 'share_allocated', label: 'Share % Allocated', group: 'Directors', type: 'number', defaultHidden: true, get: (c) => Math.round((c.directors ?? []).reduce((s, d) => s + (d.share_percentage ?? 0), 0) * 100) / 100 },
  ...
  const filters: ReportFilter<Company>[] = [
    ...
    { key: 'exporter_type', label: 'Exporter type', kind: 'select', options: distinctOptions(companies, (c) => c.exporter_type), get: (c) => c.exporter_type },
